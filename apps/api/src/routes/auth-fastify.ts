...
export async function authRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post('/register', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!parsed.success) return reply.status(400).send({ message: 'Validation failed', details: parsed.error.errors });
    ...
    try {
      ...
      const user = await prisma.user.create({ data: { email, passwordHash, role: 'VIEWER', orgId: org.id } });
  ...
  server.post('/login', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const tokens = makeTokenPair({ id: user.id, email: user.email, role: user.role, orgId: user.orgId });
      return reply.status(200).send({ user: formatUser(user), tokens });
    } catch (err: any) {
      server.log.error(err);
  ...
  server.post('/refresh', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!parsed.success) return reply.status(400).send({ message: 'Validation failed' });
    const { refreshToken } = parsed.data;
    try {
      const decoded = verifyRefreshToken(refreshToken);
      const stored = refreshTokenStore.get(refreshToken);
      if (!stored || stored.userId !== decoded.sub) return reply.status(401).send({ message: 'Invalid or expired refresh token' });
      refreshTokenStore.delete(refreshToken);
      let user: any = null;
      if (prisma) user = await prisma.user.findUnique({ where: { id: decoded.sub as string } });
  ...
  server.post('/logout', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if (body?.refreshToken) refreshTokenStore.delete(body.refreshToken);
    } catch { /* best effort */ }
  ...
  server.get('/me', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!authHeader?.startsWith('Bearer ')) return reply.status(401).send({ message: 'Missing authorization token' });
    try {
